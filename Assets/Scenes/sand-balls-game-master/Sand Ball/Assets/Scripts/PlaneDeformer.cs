...
public class PlaneDeformer : MonoBehaviour
{
    ...
    public void deformThePlane(Vector3 positionToDeform)
    {
        ...
        for (int i = 0; i < verts.Length; i++)
        {
            ...
            if (dist < radiusOfDeformation)
            {
                ...
                verts[i].y = Mathf.Clamp(verts[i].y, -1f/ dist * Depth, 0f);
        ...
        if (somethingDeformed)
        {
            mesh.vertices = verts;
            col.sharedMesh = mesh;
            //Instantiate(cylinderPrefab, new Vector3(hitpoint.x, hitpoint.y, hitpoint.z - 0.11f), Quaternion.Euler(-180f, 0f, 0f));
        }
    }
    ...
    public void puthole(Vector3 positionToDeform, float radius)
    {
        ...
        if (somethingDeformed)
        {
            mesh.vertices = verts;
            col.sharedMesh = mesh;
            Instantiate(cylinderPrefab, new Vector3(hitpoint.x, hitpoint.y, hitpoint.z + 0.9f), Quaternion.Euler(-180f, 0f, 0f));
        }
    }
}
